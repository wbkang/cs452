...
fixed tref = fixed_new(115);
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A11");
	end_node = (track_node*) lookup_get(track_lookup, "C7");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A15");
	end_node = (track_node*) lookup_get(track_lookup, "A11");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A4");
	end_node = (track_node*) lookup_get(track_lookup, "B16");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A6");
	end_node = (track_node*) lookup_get(track_lookup, "B10");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A7");
	end_node = (track_node*) lookup_get(track_lookup, "B12");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "A9");
	end_node = (track_node*) lookup_get(track_lookup, "B8");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "B1");
	end_node = (track_node*) lookup_get(track_lookup, "D14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "B14");
	end_node = (track_node*) lookup_get(track_lookup, "D16");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "B16");
	end_node = (track_node*) lookup_get(track_lookup, "C10");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "B16");
	end_node = (track_node*) lookup_get(track_lookup, "C5");
	ASSERT(start_node, "failed to lookup B16");
	ASSERT(strcmp(start_node->name, "B16") == 0, "Expected: B16, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "B6");
	end_node = (track_node*) lookup_get(track_lookup, "C12");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C10");
	end_node = (track_node*) lookup_get(track_lookup, "B1");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C10");
	end_node = (track_node*) lookup_get(track_lookup, "C2");
	ASSERT(start_node, "failed to lookup C10");
	ASSERT(strcmp(start_node->name, "C10") == 0, "Expected: C10, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C12");
	end_node = (track_node*) lookup_get(track_lookup, "A4");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C14");
	end_node = (track_node*) lookup_get(track_lookup, "A14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C14");
	end_node = (track_node*) lookup_get(track_lookup, "A15");
	ASSERT(start_node, "failed to lookup C14");
	ASSERT(strcmp(start_node->name, "C14") == 0, "Expected: C14, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C14");
	end_node = (track_node*) lookup_get(track_lookup, "A2");
	ASSERT(start_node, "failed to lookup C14");
	ASSERT(strcmp(start_node->name, "C14") == 0, "Expected: C14, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C15");
	end_node = (track_node*) lookup_get(track_lookup, "D12");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C2");
	end_node = (track_node*) lookup_get(track_lookup, "E2");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C5");
	end_node = (track_node*) lookup_get(track_lookup, "C15");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C5");
	end_node = (track_node*) lookup_get(track_lookup, "C3");
	ASSERT(start_node, "failed to lookup C5");
	ASSERT(strcmp(start_node->name, "C5") == 0, "Expected: C5, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C7");
	end_node = (track_node*) lookup_get(track_lookup, "C3");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C7");
	end_node = (track_node*) lookup_get(track_lookup, "E11");
	ASSERT(start_node, "failed to lookup C7");
	ASSERT(strcmp(start_node->name, "C7") == 0, "Expected: C7, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C8");
	end_node = (track_node*) lookup_get(track_lookup, "A6");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C8");
	end_node = (track_node*) lookup_get(track_lookup, "A7");
	ASSERT(start_node, "failed to lookup C8");
	ASSERT(strcmp(start_node->name, "C8") == 0, "Expected: C8, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "C8");
	end_node = (track_node*) lookup_get(track_lookup, "A9");
	ASSERT(start_node, "failed to lookup C8");
	ASSERT(strcmp(start_node->name, "C8") == 0, "Expected: C8, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D1");
	end_node = (track_node*) lookup_get(track_lookup, "B14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D1");
	end_node = (track_node*) lookup_get(track_lookup, "C1");
	ASSERT(start_node, "failed to lookup D1");
	ASSERT(strcmp(start_node->name, "D1") == 0, "Expected: D1, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D10");
	end_node = (track_node*) lookup_get(track_lookup, "D5");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D10");
	end_node = (track_node*) lookup_get(track_lookup, "D8");
	ASSERT(start_node, "failed to lookup D10");
	ASSERT(strcmp(start_node->name, "D10") == 0, "Expected: D10, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D12");
	end_node = (track_node*) lookup_get(track_lookup, "E11");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D14");
	end_node = (track_node*) lookup_get(track_lookup, "E14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D16");
	end_node = (track_node*) lookup_get(track_lookup, "E14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D4");
	end_node = (track_node*) lookup_get(track_lookup, "B6");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D5");
	end_node = (track_node*) lookup_get(track_lookup, "E6");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "D8");
	end_node = (track_node*) lookup_get(track_lookup, "E8");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E1");
	end_node = (track_node*) lookup_get(track_lookup, "B14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E11");
	end_node = (track_node*) lookup_get(track_lookup, "D10");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E14");
	end_node = (track_node*) lookup_get(track_lookup, "E9");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E15");
	end_node = (track_node*) lookup_get(track_lookup, "C12");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E2");
	end_node = (track_node*) lookup_get(track_lookup, "E15");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E3");
	end_node = (track_node*) lookup_get(track_lookup, "D1");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E6");
	end_node = (track_node*) lookup_get(track_lookup, "D4");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E6");
	end_node = (track_node*) lookup_get(track_lookup, "E3");
	ASSERT(start_node, "failed to lookup E6");
	ASSERT(strcmp(start_node->name, "E6") == 0, "Expected: E6, what i got: %s", start_node->name); 
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E8");
	end_node = (track_node*) lookup_get(track_lookup, "C14");
	...
	path.depth = 0;
	start_node = (track_node*) lookup_get(track_lookup, "E9");
	end_node = (track_node*) lookup_get(track_lookup, "D5");
