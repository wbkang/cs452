...
static inline void sensor_pic_def(char mod, int id1, enum direction dir1, int id2, enum direction dir2, int row,
		int col) {
...
static void print_landmark(void* s) {
	...
	if (state->last_node && state->last_node != node) {
		...
		while (curnode != node) {
			ASSERT(curedge, "curedge is null. finding %s to %s, curnode:%s total_beta: %F", state->last_node->name, node->name, curnode->name, total_beta);
